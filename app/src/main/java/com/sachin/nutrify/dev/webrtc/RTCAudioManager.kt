...
class RTCAudioManager(context: Context) {
    ...
    private inner class WiredHeadsetReceiver() : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent) {
            ...
            d(TAG, "WiredHeadsetReceiver.onReceive"
...
//        }
        ...
        audioFocusChangeListener =
            OnAudioFocusChangeListener { focusChange ->
                ...
                d(TAG, "onAudioFocusChange: $typeOfChange")
            }
        ...
        if (result == AudioManager.AUDIOFOCUS_REQUEST_GRANTED) {
            ...
            d(TAG, "Audio focus request failed")
        }
    ...
    fun stop() {
        ...
        d(TAG, "AudioManager stopped")
    }

    /** Changes selection of the currently active audio device.  */
    private fun setAudioDeviceInternal(device: AudioDevice?) {
        d(TAG, "setAudioDeviceInternal(device=$device)")
        if (audioDevices.contains(device)) {
            when (device) {
                ...
                else -> d(TAG, "Invalid audio device selection")
            }
        }
    ...
    fun setDefaultAudioDevice(defaultDevice: AudioDevice?) {
        ...
        when (defaultDevice) {
            ...
            else -> d(TAG, "Invalid default audio device selection")
        }
    ...
    fun updateAudioDeviceState() {
        ...
        d(TAG, "--- updateAudioDeviceState done")
    }
